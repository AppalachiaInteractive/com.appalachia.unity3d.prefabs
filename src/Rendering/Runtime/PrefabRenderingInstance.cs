...
namespace Appalachia.Prefabs.Rendering.Runtime
{
    ...
    public class PrefabRenderingInstance : InternalBase<PrefabRenderingInstance>, IDisposable
    {
        ...
        [ReadOnly, SerializeField, SmartLabel, ToggleLeft]
        ...
        [ReadOnly, SerializeField, InlineProperty, SmartLabel]
        public InstanceState previousState;

        [ReadOnly, SerializeField, InlineProperty, SmartLabel(AlignWith = nameof(previousState))]
            ...
            out bool pushGpuiMatrices)
        {
            using (_PRF_ApplyNewInstanceState.Auto())
            {
                ...
                {
                    ex.LogException($"Error moving [{metadata.prototype.prefabObject.name}] from [{currentState}] to [{nextState}]: {ex.Message}");
            ...
            out bool pushGpuiMatrices)
        {
            using (_PRF_ApplyStateChange.Auto())
            {
                ...
                var curr_physx = curr.physics == InstancePhysicsState.Enabled ? State.Enabled : State.Disabled;
                var curr_inter = curr.interaction == InstanceInteractionState.Enabled ? State.Enabled : State.Disabled;
                var curr_rendr = curr.rendering != InstanceRenderingState.Disabled ? State.Enabled : State.Disabled;
                var curr_meshR = curr.rendering == InstanceRenderingState.MeshRenderers ? State.Enabled : State.Disabled;
                var curr_gpuiR = curr.rendering == InstanceRenderingState.GPUInstancing ? State.Enabled : State.Disabled;

                curr_physx = curr_physx.on() || ((firstPhysicsCollider != null) && firstPhysicsCollider.enabled) ? State.Enabled : State.Disabled;
                curr_inter = curr_inter.on() || (rigidbody != null) || ((firstInteractionCollider != null) && firstInteractionCollider.enabled)
                    ? State.Enabled
                    : State.Disabled;
                curr_rendr = curr_rendr.on() || ((lodGroup != null) && lodGroup.enabled) ? State.Enabled : State.Disabled;
                curr_meshR = curr_meshR.on() || ((lodGroup != null) && lodGroup.enabled) ? State.Enabled : State.Disabled;

                var next_physx = next.physics == InstancePhysicsState.Enabled ? State.Enabled : State.Disabled;
                var next_inter = next.interaction == InstanceInteractionState.Enabled ? State.Enabled : State.Disabled;
                var next_rendr = next.rendering != InstanceRenderingState.Disabled ? State.Enabled : State.Disabled;
                var next_meshR = next.rendering == InstanceRenderingState.MeshRenderers ? State.Enabled : State.Disabled;
                var next_gpuiR = next.rendering == InstanceRenderingState.GPUInstancing ? State.Enabled : State.Disabled;
                ...
                var shouldInstantiate = !instantiated && (change_physx.turnOn() || change_inter.turnOn() || change_rendr.turnOn());
                ...
                var anyTurningOff = change_physx.turnOff() || change_inter.turnOff() || change_meshR.turnOff();
                ...
                {
                    element.matrices_noGameObject_OWNED[instanceIndex] =
                        change_gpuiR.turnOn() ? element.matrices_current[instanceIndex] : float4x4.zero;
                    pushGpuiMatrices = true;
                }
                ...
                pushGpuiMatrices = pushGpuiMatrices || (curr == InstanceState.NotSet) || (next == InstanceState.NotSet);
            }
        }
            ...
            GameObject prefabInstanceRoot)
        {
            using (_PRF_Instantiate.Auto())
            {
                using (_PRF_Instantiate_Prototypes.Auto())
                {
                    ...
                    {
                        element.prototypeTemplate = metadata.originalPrefab.InstantiatePrefab(prefabInstanceRoot.transform, _matrix_trs_zero);
                    ...
                    if (element.prototypeTemplate == null)
                    {
                        throw new NotSupportedException($"Template prototype could not be instantiated for {metadata.prototype.name}.");
                    }
                }
        ...
        private static readonly ProfilerMarker _PRF_CreatePrefabPool = new ProfilerMarker(_PRF_PFX + nameof(CreatePrefabPool));

        private ObjectPool<GameObject> CreatePrefabPool(PrefabRenderingSet set, RuntimePrefabRenderingElement element, GameObject prefabInstanceRoot)
        {
        ...
        private static readonly ProfilerMarker _PRF_CreateModelInstance = new ProfilerMarker(_PRF_PFX + nameof(CreateModelInstance));

        private static GameObject CreateModelInstance(PrefabRenderingSet set, RuntimePrefabRenderingElement element, GameObject prefabInstanceRoot)
        {
        ...
        private static void InitializePrefabRenderingInstanceBehaviour(PrefabRenderingInstanceBehaviour ib, PrefabRenderingSet set)
        {
            ib.set = set;
        ...
        private void InitializeInteractions()
        {
            using (_PRF_InitializeInteractions.Auto())
            {
                ...
                if (firstInteractionCollider == null)
                {
                    ...
                    firstInteractionCollider = interactionColliders.Length > 0 ? interactionColliders[0] : null;
                }
            }
        }

        private void UpdateInteractions(GPUInstancerPrototypeMetadata metadata, InstanceInteractionState state)
        {
...
#region ProfilerMarkers
        ...
        private static readonly ProfilerMarker _PRF_ApplyNewInstanceState = new ProfilerMarker(_PRF_PFX + nameof(ApplyNewInstanceState));
        private static readonly ProfilerMarker _PRF_ApplyStateChange = new ProfilerMarker(_PRF_PFX + nameof(ApplyStateChange));
        private static readonly ProfilerMarker _PRF_AssignElementProperties = new ProfilerMarker(_PRF_PFX + nameof(AssignElementProperties));
        private static readonly ProfilerMarker _PRF_Dispose = new ProfilerMarker(_PRF_PFX + nameof(Dispose));
        private static readonly ProfilerMarker _PRF_InitializeInteractions = new ProfilerMarker(_PRF_PFX + nameof(InitializeInteractions));
        private static readonly ProfilerMarker _PRF_InitializePhysics = new ProfilerMarker(_PRF_PFX + nameof(InitializePhysics));

        private static readonly ProfilerMarker _PRF_InitializePhysics_ColliderQuery =
            new ProfilerMarker(_PRF_PFX + nameof(InitializePhysics) + ".ColliderQuery");

        private static readonly ProfilerMarker _PRF_InitializePhysics_ConvexCheck =
            new ProfilerMarker(_PRF_PFX + nameof(InitializePhysics) + ".ConvexCheck");

        private static readonly ProfilerMarker _PRF_InitializePhysics_NullCheck =
            new ProfilerMarker(_PRF_PFX + nameof(InitializePhysics) + ".NullCheck");

        private static readonly ProfilerMarker _PRF_InitializeRendering = new ProfilerMarker(_PRF_PFX + nameof(InitializeRendering));

        private static readonly ProfilerMarker _PRF_InitializeRendering_LODGroup =
            new ProfilerMarker(_PRF_PFX + nameof(InitializeRendering) + ".LODGroup");

        private static readonly ProfilerMarker _PRF_InitializeRendering_Renderers =
            new ProfilerMarker(_PRF_PFX + nameof(InitializeRendering) + ".Renderers");

        private static readonly ProfilerMarker _PRF_Instantiate = new ProfilerMarker(_PRF_PFX + nameof(Instantiate));
        private static readonly ProfilerMarker _PRF_Instantiate_Clone = new ProfilerMarker(_PRF_PFX + nameof(Instantiate) + ".Clone");

        private static readonly ProfilerMarker _PRF_Instantiate_InstanceBehaviour =
            new ProfilerMarker(_PRF_PFX + nameof(Instantiate) + ".InstanceBehaviour");

        private static readonly ProfilerMarker _PRF_Instantiate_Integrate = new ProfilerMarker(_PRF_PFX + nameof(Instantiate) + ".Integrate");
        private static readonly ProfilerMarker _PRF_Instantiate_PrefabPool = new ProfilerMarker(_PRF_PFX + nameof(Instantiate) + ".PrefabPool");
        private static readonly ProfilerMarker _PRF_Instantiate_Prototypes = new ProfilerMarker(_PRF_PFX + nameof(Instantiate) + ".Prototypes");
        private static readonly ProfilerMarker _PRF_ReturnInstanceToPool = new ProfilerMarker(_PRF_PFX + nameof(ReturnInstanceToPool));
        private static readonly ProfilerMarker _PRF_ShouldDelayStateChange = new ProfilerMarker(_PRF_PFX + nameof(ShouldDelayStateChange));
        private static readonly ProfilerMarker _PRF_Teardown = new ProfilerMarker(_PRF_PFX + nameof(Teardown));
        private static readonly ProfilerMarker _PRF_UpdateInteractions = new ProfilerMarker(_PRF_PFX + nameof(UpdateInteractions));
        private static readonly ProfilerMarker _PRF_UpdatePhysics = new ProfilerMarker(_PRF_PFX + nameof(UpdatePhysics));
        private static readonly ProfilerMarker _PRF_UpdateRendering = new ProfilerMarker(_PRF_PFX + nameof(UpdateRendering));

        private static readonly ProfilerMarker _PRF_UpdateRendering_CheckEnabled =
            new ProfilerMarker(_PRF_PFX + nameof(UpdateRendering) + ".CheckEnabled");

        private static readonly ProfilerMarker _PRF_UpdateRendering_CheckLayer =
            new ProfilerMarker(_PRF_PFX + nameof(UpdateRendering) + ".CheckLayer");

        private static readonly ProfilerMarker _PRF_UpdateRendering_LODGroup = new ProfilerMarker(_PRF_PFX + nameof(UpdateRendering) + ".LODGroup");

        private static readonly ProfilerMarker _PRF_UpdateRendering_OverrideLighting =
            new ProfilerMarker(_PRF_PFX + nameof(UpdateRendering) + ".OverrideLighting");
